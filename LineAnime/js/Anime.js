  ...
  var y = (function () {
    function e(t, n) {
      ...
      !(function (e, t) {
        ...
        (this.randomPoints = function () {
          return m(o.c.count).map(function () {
            return {
              ...
              xa: (2 * Math.random() - 1) * o.getParticleSpeed(),
              ya: (2 * Math.random() - 1) * o.getParticleSpeed(),
              max: 6e3,
              baseXa: 2 * Math.random() - 1, // 存储基础速度分量
              baseYa: 2 * Math.random() - 1  // 存储基础速度分量
            };
          });
        }),
        (this.el = t),
        ...
        (this.lastConfigCheck = performance.now()),
        (this.configCheckInterval = 500), // 每500毫秒检查一次配置变化
        this.bindEvent(),
    ...
    return (
      p(e, [
        {
          key: "getParticleSpeed",
          value: function () {
            return ConfigManager.get('particleSpeed');
          }
        },
        {
          key: "updateParticleSpeeds",
          value: function () {
            var currentSpeed = this.getParticleSpeed();
            this.points.forEach(function (point) {
              // 基于存储的基础速度分量重新计算当前速度
              point.xa = point.baseXa * currentSpeed;
              point.ya = point.baseYa * currentSpeed;
            });
          }
        },
        {
          ...
          value: function () {
            ...
            if (currentTime - this.lastConfigCheck > this.configCheckInterval) {
              ...
              // 检查粒子速度配置是否变化
              var newSpeed = this.getParticleSpeed();
              if (newSpeed !== this.lastParticleSpeed) {
                this.lastParticleSpeed = newSpeed;
        ...
        },
        {
          ...
          value: function () {
            ...
            var speed = deltaTime / (1000 / 60); // 基于60FPS为标准
            this.lastTime = currentTime;
            
            // 限制最大速度因子，避免页面切换后动画过快
            if (speed > 2) speed = 1;
            ...
            var s = void 0,
              ...
              d = void 0,
              v = void 0;
            const hue = (Date.now() * ConfigManager.get('colorChangeSpeed', 0.001)) % 360;
            const opacity = ConfigManager.get('opacity', 0.5);
            const color = "hsla(" + hue + ", 100%, 50%, " + opacity + ")";
            const pointSize = ConfigManager.get('pointSize', 2);
            ...
            r.forEach(function (r, f) {
              ...
              // 绘制粒子
              t.fillStyle = color;
              t.fillRect(r.x - pointSize / 2, r.y - pointSize / 2, pointSize, pointSize);
              ...
              for (u = f + 1; u < a.length; u++) {
                null !== (s = a[u]).x &&
                  ...
                  (v = l * l + d * d) < s.max &&
                    (s === i &&
                      v >= s.max / 2 &&
                      ((r.x -= 0.03 * l * speed), (r.y -= 0.03 * d * speed)),
                    (c = (s.max - v) / s.max),
                    t.beginPath(),
                    (t.lineWidth = c / 2 * pointSize),
                    (t.strokeStyle = color),
                    t.moveTo(r.x, r.y),
                    t.lineTo(s.x, s.y),
                    t.stroke()));
